...
async function scrapeProxy(retries = 2) {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (!data.trim()) throw new Error('Konten kosong');
...
`;
  ...
  const buttons = [
    ...
    [{ text: "Developer", url: "https://t.me/PSYKOOFFICIAL" }],
  ];
  ...
  if (parts.length < 3 || parts.length > 4)
    return bot.sendMessage(
      ...
      "⚙️ Format salah!\nGunakan salah satu format berikut:\n\n" +
        "• `/addkode psyko123,5,1h`\n  → Tanpa batas pengguna\n\n" +
        "• `/addkode psykoVIP,10,1d,5`\n  → Batas 5 pengguna\n",
      { parse_mode: "Markdown" }
    );
...
🕒 ${time} | 🔋 ${battery}% | 📡 ${carrier}`,
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "⌈ DEVELϴPER ⌋", url: "https://t.me/PSYKOOFFICIAL" }]
        ]
      }
    });
  } catch (err) {
